// server/src/scripts/reindex-cgi-2025.ts
...
const DATA_DIR = path.resolve(process.cwd(), 'data/cgi/2025');

const client = new QdrantClient({
  url: process.env.QDRANT_URL || 'http://localhost:6333',
});

const COLLECTION_NAME = 'cgi_2025';
...
/**
 * Crée ou recrée la collection CGI 2025
 */
...
/**
 * Charge tous les fichiers JSON du dossier 2025
 */
...
async function indexArticles(files: CGIFile[]): Promise<void> {
  ...
  for (const file of files) {
    ...
    for (const article of articles) {
      ...
      const articleKey = `${article.article}-${meta.chapitre_titre}`;
      if (seenArticles.has(article.article)) {
        console.log(`  ⏭️  Skip doublon: ${article.article}`);
        continue;
      }
      ...
      try {
          ...
          points: [
            {
              ...
              payload: {
                articleId: id,
                numero: article.article.replace('Art. ', ''),
                titre: article.titre || '',
        ...
        totalIndexed++;
        console.log(`  ✅ ${article.article}: ${article.titre || '(sans titre)'}`);
...
async function verifyIndexation(): Promise<void> {
  ...
  // Test de recherche
  const testQueries = [
    { query: 'catégories de revenus IRPP', expected: '1' },
    { query: 'résidence fiscale vingt-quatre mois', expected: '2' },
    { query: 'barème IRPP taux', expected: '95' },
  ];
...
async function main(): Promise<void> {
  ...
  console.log('   RÉINDEXATION CGI 2025');
  console.log('='.repeat(60));
  console.log('');
